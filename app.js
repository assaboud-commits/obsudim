// Mini App v10 — календарь и участники (без напоминаний)
...
function fmtDateRange(a,b){
  ...
  if(same) return da.toLocaleDateString(STATE.lang==='ru'?'ru-RU':'en-GB',opts);
...
function classify(it){
  ...
  if(n.includes('гран-при финал')||n.includes('grand prix final')) return 'gpf';
  if(n.includes('гран-при')||n.includes('grand prix')) return 'gp';
  if(n.includes('мир')||n.includes('world')) return 'worlds';
  if(n.includes('европ')||n.includes('europe')) return 'euros';
  if(n.includes('олимп')||n.includes('olymp')) return 'oly';
  return '';
}
...
function chips(it){
  ...
  const base = colorForClass(cls)+'cc';
  ...
  return `
    <div class="card view" style="border-top:4px solid ${topBorder}">
      ...
      <div style="margin-top:10px">
        <a class="btn" href="${it.url||'#'}" target="_blank">🌐 ${t('official')}</a>
